...
export const some: <A>(a: NonNullable<A>) => Optional<A> = (value) => value as any;
...
export const getOrElse: <A, B>(onNone: () => B) => (ma: Optional<A>) => A | B =
	(onNone) => (ma) => (isSome(ma) ? (ma as any) : onNone());

export const map: <A, B>(f: (a: A) => B) => (ma: Optional<A>) => Optional<B> =
	(f) => (ma) => {
...
export const flap: <A>(a: A) => <B>(fab: Optional<(a: A) => B>) => Optional<B> =
