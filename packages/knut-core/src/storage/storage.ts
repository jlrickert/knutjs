...
import { getUserCacheDir } from '../internal/systemUtils.js';
...
export type GenericStorage = {
	/**
	 * resolve the path relative to the root of the file system
	 **/
...
export const loadCacheStorage = async (): Promise<GenericStorage> => {
	const cache = await getUserCacheDir();
	switch (currentEnvironment) {
		case 'dom': {
			const storage = WebStorage.create();
			return storage.child(cache);
		}
		case 'node': {
			const storage = new FsStorage(cache);
			return storage;
		}
		default: {
			const storage = MemoryStorage.create();
