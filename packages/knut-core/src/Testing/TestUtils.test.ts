...
import { Future, FutureResult, Result, optionalT } from '../Utils/index.js';

for await (const { name, loadBackend: getBackend } of TestUtils.backends) {
		...
		it('knut should be able to load a keg', async () => {
			...
			const load = async (kegalias: string) => {
				...
				const keg = pipe(
					...
					FutureResult.chain((a) => Keg.fromStorage(a)),
